    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div class="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-2xl p-6 shadow-lg shadow-indigo-500/25 text-white">
                    ...
                    <div class="mt-4 flex gap-4 text-xs">
                        <span>{{ $stats['total_teachers'] }} Profs</span>
                        <span>{{ $stats['total_managers'] }} Managers</span>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="p-0">
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                ...
                                @empty
                                <tr>
                                    <td colspan="2" class="px-4 py-6 text-center text-sm text-slate-400">Aucune note</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                        <h3 class="font-semibold text-slate-800">Dernières Évaluations</h3>
                    </div>
                    <div class="p-0">
                        <table class="w-full">
                            <thead class="bg-slate-50">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                ...
                                @empty
                                <tr>
                                    <td colspan="2" class="px-4 py-6 text-center text-sm text-slate-400">Aucune évaluation</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                        <h3 class="font-semibold text-slate-800">Derniers Utilisateurs</h3>
                    </div>
                    <div class="p-0">
                        <table class="w-full">
                            <thead class="bg-slate-50">
                                <tr>
                                    <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Nom</th>
                                    <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Email</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100">
                                @forelse($recent_users as $user)
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    <td class="px-4 py-3 text-sm text-slate-700">{{ $user->nom }} {{ $user->prenom }}</td>
                                    <td class="px-4 py-3 text-sm text-slate-500">{{ $user->email }}</td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="2" class="px-4 py-6 text-center text-sm text-slate-400">Aucun utilisateur</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <a href="{{ route('utilisateurs.create') }}" class="flex items-center gap-3 p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition-colors">
                            ...
                            <span class="text-sm font-medium text-slate-700">Ajouter Utilisateur</span>
                        </a>
                        <a href="{{ route('classes.create') }}" class="flex items-center gap-3 p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition-colors">
                            ...
                            <span class="text-sm font-medium text-slate-700">Ajouter Classe</span>
                        </a>
                        <a href="{{ route('matieres.create') }}" class="flex items-center gap-3 p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition-colors">
                            <div class="w-10 h-10 bg-violet-100 rounded-lg flex items-center justify-center">
                                <svg class="w-5 h-5 text-violet-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                            </div>
                            <span class="text-sm font-medium text-slate-700">Ajouter Matière</span>
                        </a>
                        <a href="{{ route('evaluations.create') }}" class="flex items-center gap-3 p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition-colors">
                            ...
                            <span class="text-sm font-medium text-slate-700">Ajouter Évaluation</span>
                        </a>
                    </div>
                </div>

                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    <h3 class="font-semibold text-slate-800 mb-4">Statistiques Générales</h3>
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center justify-between">
                            <span class="text-sm text-slate-600">Matières</span>
                            <span class="font-semibold text-slate-800">{{ $stats['total_matieres'] }}</span>
                        </div>
                        <div class="flex items-center justify-between">
